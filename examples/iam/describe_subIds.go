...
func main() {
	...
	request := iam.NewDescribeSubIdsRequest()
	request.MasterId = "511008144529233333"

	// 通过client对象调用想要访问的接口，需要传入请求对象
	response, err := client.DescribeSubIds(request)
	...
	// 打印返回的json字符串
	fmt.Printf("%v, %v", response.Response.SubIds, response.Response.RequestId)
}
